		...
		if(null!=sArr && '' != sArr){
			for(var i = 0; i < sArr.length; i++){
				if(uSinaEmotionsHt.containsKey(sArr[i])) {
					var reStr = "<img src=\"" + uSinaEmotionsHt.get(sArr[i]) + "\" height=\"25\" width=\"25\" />";
	...
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			if($('#emotions .categorys')[0]){
				$('#emotions').css({top: -247, left: 0});
				$('#emotions').toggle();
				return;
			}
			$('.fpc-bot').append('<div id="emotions"></div>');
			$('#emotions').css({top: -247, left: 0});
		...
		function showEmotions(){
			...
			for(var i = 0;  i < emotions[category].length; ++i){
				$('#emotions .container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="25" height="25" /></a>'));
			}
